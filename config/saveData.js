...
const dateFormat = (num) => {
  ...
  // Add the number of days (44,815) to the base date

  // Subtract 2 day because Excel starts counting from 2 (not 0)
  const resultDate = new Date(
    baseDate.getTime() + (num - 2) * 24 * 60 * 60 * 1000
  );
...
const saveData = async () => {
  ...
  if (allData.resa && Array.isArray(allData.resa)) {
    ...
    for (const item of resaArray) {
      // Assuming your Excel sheet has the fields that match your Resa schema
      //   const name = item[0];
      //   let findResa = await Resa.findOne({ name });
      //   if (findResa) continue;
      ...
      const resaData = {
        ...
        htl_region: item[7] || "",
        service_date:
          item[8] && Number(item[8]) ? dateFormat(Number(item[8])) : "",
        endofservice:
          item[9] && Number(item[9]) ? dateFormat(Number(item[9])) : "",
        no_of_ngts: item[10] || "",
        adult: item[11] || "",
        child: item[12] || "",
        infant: item[13] || "",
        ...
        status: item[31] || "",
        effect_date:
          item[32] && Number(item[32]) ? dateFormat(Number(item[32])) : "",
        inv_no: item[33] || "",
      };
